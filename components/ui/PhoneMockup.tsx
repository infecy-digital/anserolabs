...
interface RetellWebClient {
  startCall: (request: { accessToken: string; sampleRate?: number }) => Promise<void>;
  stopCall: () => void;
  ...
  useEffect(() => {
    ...
    return () => {
      if (retellClientRef.current) {
        retellClientRef.current.stopCall(); // Changed from stopConversation
      }
    };
  }, []);
    ...
    if (isConnected) {
      ...
      try {
        ...
        if (retellClientRef.current && data.access_token) {
          ...
          await retellClientRef.current.startCall({ // Changed from startConversation
